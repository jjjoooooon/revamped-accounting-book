  ...
  return (
    <div id="bulk-print-container" className="hidden print:block">
      ...
      {receipts.map((data, index) => (
        <div
          key={index}
          className="receipt-page font-mono text-sm leading-tight text-black"
        >
...
export default function BulkCollectionPage() {
  ...
  const [endMonth, setEndMonth] = useState(
    format(addMonths(new Date(), 6), "yyyy-MM"),
  );
  ...
  const fetchMembers = async () => {
    ...
    try {
      const res = await fetch(
        `/api/sanda/bulk-status?startMonth=${startMonth}&endMonth=${endMonth}`,
      );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage payments across multiple months.
          </p>
        </div>
                  ...
                  {loading ? (
                    ...
                    filteredMembers.map((member) => (
                      <TableRow key={member.memberId}>
                        <TableCell
                          className="font-medium sticky left-0 bg-background z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] cursor-pointer hover:bg-muted/50"
                        ...
                        {months.map((month) => {
                          const status =
                            member.payments[month]?.status || "pending";
                          const isSelected =
                            !!selectedPayments[`${member.memberId}_${month}`];
                          ...
                          return (
                            <TableCell
                              ...
                              className={`text-center p-1 border-l cursor-pointer transition-colors
                                                            ...
                                                            `}
                              onClick={() =>
                                toggleSelection(member.memberId, month, status)
                              }
                            >
                              {status === "paid" ? (
      ...
      <Dialog open={isPrintConfirmOpen} onOpenChange={setIsPrintConfirmOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {printReceipts.length} payments have been processed. Do you want
              to print the receipts now?
            </DialogDescription>
          </DialogHeader>
        ...
        {getSelectedCount() > 0 && (
          <Dialog open={isConfirmOpen} onOpenChange={setIsConfirmOpen}>
            <DialogTrigger asChild>
              <Button
                size="lg"
                className="shadow-lg animate-in fade-in slide-in-from-bottom-4 bg-emerald-600 hover:bg-emerald-700 text-white border-emerald-700"
              >
            ...
            <DialogContent className="max-w-md border-emerald-100">
              <DialogHeader>
                ...
                <DialogDescription>
                  You are about to record payments for{" "}
                ...
                <div className="flex items-center gap-3 text-sm">
                  ...
                  <span>
                    ...
                    {selectedMember.startDate
                      ? format(new Date(selectedMember.startDate), "PPP")
                      : "Unknown"}
                  </span>
                </div>
                <div className="flex items-center gap-3 text-sm">
                  ...
                  <span>
                    Monthly Sanda:{" "}
                    <strong>{selectedMember.amount.toFixed(2)}</strong>
                  </span>
                </div>
              </div>
            </div>
