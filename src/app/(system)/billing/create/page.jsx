...
const SandaReceipt = ({ data, settings }) => {
  ...
  return (
    <div
      id="sanda-receipt"
      className="hidden print:block p-2 bg-white text-black font-mono text-sm leading-tight"
    >
      <style jsx global>{`
        @media print {
      ...
      <div className="flex justify-between items-center my-4">
        ...
        <span className="font-bold text-xl">
          Rs. {Number(data.totalPaid).toLocaleString()}
        </span>
      </div>
...
export default function SandaCollectionPage() {
  ...
  const handleSelectMember = async (id) => {
    ...
    try {
      ...
      // Calculate Arrears
      const totalArrears = formattedInvoices.reduce(
  ...
  const onSubmit = async (data) => {
    ...
    const covered = allocationPreview
      .filter((a) => a.paidForThis > 0)
      .map((a) => ({
        month: a.month,
        paid: a.paidForThis,
        invoiceId: a.id,
    ...
    if (covered.length === 0) {
      if (pendingInvoices.length === 0) {
        toast.error("No pending invoices to pay.", {
          description: "Please generate invoices for this member first.",
        });
      } else {
        toast.error("Amount does not cover any invoices.", {
    ...
    try {
      ...
      toast.success("Payment Recorded", {
        description: `Rs. ${data.amount} collected.`,
      });
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-slate-500 text-sm">
              Collect monthly subscription payments from members.
            </p>
          </div>
          ...
          <Popover open={openSearch} onOpenChange={setOpenSearch}>
            <PopoverTrigger asChild>
              <Button
                variant="outline"
                role="combobox"
                className="w-[300px] justify-between bg-white h-11 border-slate-300 shadow-sm text-slate-600"
              >
                {selectedMember
                  ? selectedMember.name
                  : "Find Member (ID or Name)..."}
                    ...
                    <div className="flex items-start gap-4 relative z-10">
                      ...
                      <div>
                        ...
                        <div className="mt-3 flex gap-3">
                          ...
                          >
                            {selectedMember.arrears > 0
                              ? `Arrears: Rs. ${selectedMember.arrears}`
                              : "Up to Date"}
                          </Badge>
                          <Badge
                            variant="outline"
                      ...
                      >
                        ...
                        {selectedMember.arrears > 0 && (
                          <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
                            <Button
                              ...
                              className="text-xs bg-emerald-50 text-emerald-700 border-emerald-100 hover:bg-emerald-100"
                            >
                            ...
                            <Button
                              ...
                              className="text-xs bg-rose-50 text-rose-700 border-rose-100 hover:bg-rose-100"
                            >
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            render={({ field }) => (
                              <FormItem className="flex items-center space-x-2 space-y-0">
                                ...
                                <FormLabel className="font-normal text-slate-600 flex items-center gap-2">
                                  <Printer className="w-4 h-4" /> Auto-Print
                                  Receipt
                                </FormLabel>
                              </FormItem>
                            )}
                          ...
                          <Button
                            type="submit"
                            size="lg"
                            className="bg-emerald-600 hover:bg-emerald-700 text-white min-w-[200px] shadow-lg shadow-emerald-200"
                          >
              ...
              {!selectedMember ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-slate-400 opacity-50">
                  ...
                  <p className="text-xs text-center">
                    This member has no outstanding dues.
                  </p>
                </div>
              ) : (
