...
export async function GET(request) {
  try {
    ...
    const allTransactions = [
      ...standardizedDonations,
      ...standardizedPayments,
      ...standardizedOtherIncome,
    ].sort((a, b) => new Date(b.date) - new Date(a.date));

    // 3. Calculate Stats
    const totalIncome = allTransactions.reduce((sum, t) => sum + t.amount, 0);
    const sandaTotal = standardizedPayments.reduce(
      (sum, t) => sum + t.amount,
      0,
    );
    const donationTotal = standardizedDonations.reduce(
      (sum, t) => sum + t.amount,
      0,
    );
    const otherTotal = standardizedOtherIncome.reduce(
      (sum, t) => sum + t.amount,
      0,
    );
