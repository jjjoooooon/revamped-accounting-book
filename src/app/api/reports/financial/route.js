...
export async function GET(request) {
  try {
    ...
    const pendingBillsAmount = pendingInvoices.reduce(
    ...
    expenses.forEach((e) => {
      ...
      expenseCategoryMap[catName] =
        (expenseCategoryMap[catName] || 0) + e.amount;
    });
    const expenseCategories = Object.entries(expenseCategoryMap).map(
      ([category, amount]) => ({
        category,
        amount,
      }),
