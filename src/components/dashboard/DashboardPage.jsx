...
const initialNotifications = [
  {
    id: 1,
    text: "New donation received: Rs. 50,000",
    time: "2 min ago",
    read: false,
  },
  {
    id: 2,
    text: "Monthly Electricity bill generated",
    time: "1 hour ago",
    read: false,
  },
  {
    id: 3,
    text: "Member #402 payment overdue",
    time: "5 hours ago",
    read: true,
  },
];
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-emerald-950 tracking-tight">
                  {settings?.mosqueName || "Masjidhul Haadhi"}
                </h1>
                <p className="text-xs text-emerald-600 font-medium capitalize">
                  {session?.user?.role || "Masjid Admin"}
                </p>
              </div>
            </div>
                      ...
                      <div className="max-h-[300px] overflow-y-auto">
                        {notifications.length === 0 ? (
                          ...
                          notifications.map((notif) => (
                            ...
                            >
                              <div
                                className={`mt-1.5 h-2 w-2 rounded-full flex-shrink-0 ${notif.read ? "bg-slate-300" : "bg-emerald-500"}`}
                              />
                              <div>
                    ...
                    >
                      <div className="px-4 py-2 border-b border-slate-100 mb-1">
                        <p className="text-sm font-bold text-slate-800">
                          {session?.user?.name || "Masjid Admin"}
                        </p>
                        <p className="text-xs text-slate-500">
                          {session?.user?.email || "admin@almanar.lk"}
                        </p>
                      </div>
                      <a
                        href="#"
                        className="flex items-center gap-2 px-4 py-2 text-sm text-slate-600 hover:bg-emerald-50 hover:text-emerald-700"
                      >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-500">
              Welcome back, {session?.user?.name || "Administrator"}. Here's
              what's happening.
            </p>
          </div>
          ...
          {dashboardStats.map((stat, index) => (
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                {stat.trend !== "neutral" && (
                  <div
                    className={`flex items-center text-xs font-semibold ${stat.trend === "up" ? "text-emerald-600" : "text-rose-600"} bg-opacity-10 px-2 py-1 rounded-full`}
                  >
                    {stat.trend === "up" ? (
                ...
                filteredActivities.map((activity) => (
                  ...
                  >
                    ...
                    <div className="text-right">
                      <p
                        className={`font-bold ${activity.amount.includes("+") ? "text-emerald-600" : activity.amount.includes("-") ? "text-slate-800" : "text-slate-400"}`}
                      >
                        {activity.amount}
                      </p>
                      <span
