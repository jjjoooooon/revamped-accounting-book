    ...
    <div className="w-full space-y-4">
      ...
      <div className="flex flex-col lg:flex-row gap-3 items-start lg:items-center justify-between bg-white p-1.5 rounded-xl border border-slate-200 shadow-sm transition-all hover:border-slate-300">
        ...
        <div className="flex items-center gap-2 w-full lg:w-auto px-1 pb-1 lg:pb-0 justify-end">
          ...
          <Separator orientation="vertical" className="h-6 hidden lg:block mx-1" />
            ...
            <PopoverTrigger asChild>
              <Button 
                ...
                className={cn(
                  ...
                  activeFilterCount > 0 && "bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100"
                )}
              >
                <Filter className="h-4 w-4" />
                <span>Filters</span>
                {activeFilterCount > 0 && (
                  <Badge variant="secondary" className="ml-1 h-5 min-w-[1.25rem] rounded-full px-1 flex items-center justify-center bg-emerald-200 text-emerald-800 text-[10px]">
                    {activeFilterCount}
                  </Badge>
                )}
              </Button>
            ...
            <PopoverContent className="w-[calc(100vw-2rem)] sm:w-80 p-4" align="end">
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <Button variant="ghost" size="sm" onClick={handleClearFilters} className="h-auto p-0 text-slate-500 text-xs hover:text-red-600 hover:bg-transparent">
                    Reset all
                  </Button>
                </div>
                
                <div className="space-y-2">
                  <label className="text-xs font-medium text-slate-500 uppercase tracking-wider">Entity Type</label>
                  <Select
                  ...
                  <label className="text-xs font-medium text-slate-500 uppercase tracking-wider">Action Type</label>
                   <div className="flex flex-wrap gap-2">
                      {['All', ...ACTIONS].map((action) => {
                          const isSelected = (filters.action === action) || (action === 'All' && !filters.action);
                          return (
                              <div
                                  key={action}
                                  onClick={() => onFilterChange({ action: action === 'All' ? '' : action })}
                                  className={cn(
      ...
      {activeFilterCount > 0 && (
        <div className="flex flex-wrap items-center gap-2 animate-in fade-in slide-in-from-top-1 duration-200">
            ...
            {filters.entityType && (
                <Badge variant="outline" className="bg-white border-slate-200 text-slate-700 pl-2 pr-1 py-1 gap-1 font-normal shadow-sm">
                    Entity: <span className="font-semibold text-slate-900">{filters.entityType}</span>
                    <Button variant="ghost" size="icon" className="h-4 w-4 ml-1 hover:bg-slate-100 rounded-full text-slate-400 hover:text-slate-600" onClick={() => onFilterChange({ entityType: '' })}>
                        <X className="h-3 w-3" />
                    </Button>
                </Badge>
            )}
            ...
            {filters.action && (
                <Badge variant="outline" className="bg-white border-slate-200 text-slate-700 pl-2 pr-1 py-1 gap-1 font-normal shadow-sm">
                    Action: <span className="font-semibold text-slate-900">{filters.action}</span>
                    <Button variant="ghost" size="icon" className="h-4 w-4 ml-1 hover:bg-slate-100 rounded-full text-slate-400 hover:text-slate-600" onClick={() => onFilterChange({ action: '' })}>
                        <X className="h-3 w-3" />
                    </Button>
                </Badge>
            )}
            ...
            {(filters.startDate || filters.endDate) && (
                <Badge variant="outline" className="bg-white border-slate-200 text-slate-700 pl-2 pr-1 py-1 gap-1 font-normal shadow-sm">
                    Date: <span className="font-semibold text-slate-900">
                        {filters.startDate ? format(new Date(filters.startDate), 'MMM dd') : 'Start'} 
                        {' - '} 
                        {filters.endDate ? format(new Date(filters.endDate), 'MMM dd') : 'End'}
                    </span>
                    <Button variant="ghost" size="icon" className="h-4 w-4 ml-1 hover:bg-slate-100 rounded-full text-slate-400 hover:text-slate-600" onClick={() => onFilterChange({ startDate: '', endDate: '' })}>
                        <X className="h-3 w-3" />
                    </Button>
                </Badge>
            )}

            <Button variant="ghost" size="sm" className="h-6 text-xs text-slate-500 hover:text-red-600 px-2" onClick={handleClearFilters}>
                Clear all
            </Button>
        </div>
      )}
